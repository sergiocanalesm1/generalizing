...
from generalizing_core.serializers.relation_serializer import RelationSerializer
...
def relation_list(request):
    return list(request,Relation,RelationSerializer)


@api_view(['GET', 'PUT', 'DELETE'])
def relation_detail(request, uuid):
    return detail(request,uuid,Relation,RelationSerializer)
...
def relation_user_list(request, uuid):
    ...
    serializer = RelationSerializer(list, many=True, context={'request':request})
